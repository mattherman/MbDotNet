...
{
    internal class CanDeleteSavedRequests : AcceptanceTest
    {
        ...
        public override void Run()
        {
            ...
            CreateImposter();
            CallImposter();
            VerifyImposterWasCalled(1, 1);
            DeleteSavedRequests();
            VerifyImposterWasCalled(1, 0);
        }

        private void DeleteSavedRequests()
        {
            _client.DeleteSavedRequests(ImposterPort);
        }

        private void VerifyImposterWasCalled(int expectedNumberOfRequests, int expectedNumberOfSavedRequests)
        {
            _retrievedImposter = _client.GetHttpImposter(ImposterPort);

            _retrievedImposter.NumberOfRequests.Should().Be(expectedNumberOfRequests);
            _retrievedImposter.Requests.Length.Should().Be(expectedNumberOfSavedRequests);

            if (expectedNumberOfSavedRequests > 0)
            {
